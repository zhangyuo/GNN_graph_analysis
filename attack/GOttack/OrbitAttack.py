...
class OrbitAttack(BaseAttack):
    ...
    def attack_cf(self, features, adj, labels, target_node, n_perturbations, n_influencers= 0, verbose=True, **kwargs):
        """Generate perturbations on the input graph.
    ...
    def random_sampling(self, features, adj, labels, target_node, n_perturbations, n_influencers= 0, verbose=True, **kwargs):
        """Generate perturbations on the input graph.
        ...
        all_nodes = all_nodes[all_nodes != target_node]
        self.potential_edges = np.column_stack((np.full_like(all_nodes, target_node), all_nodes)).astype("int32")
