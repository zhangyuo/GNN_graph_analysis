...
class SingleTrain(object):
    def __init__(self, args):
        ...
        if (self.args.train_on_non_zero):
            ...
            to_be_selected_nodes = np.random.choice(self.graph.idx_train, 20000, replace=False).tolist()
            num_node = 1000
            target_node_id = []
            for idx, node_id in enumerate(to_be_selected_nodes):
                target_node = node_id
                ...
                if num_node <= 0:
                    break
            # targets = np.random.choice(to_be_selected_nodes, 1000, replace=False).tolist()
    ...
    def playOneEpisode(self, playerid):
        ...
        orig_out = torch.argmax(self.model(self.players[playerid].G_orig.feats.to(self.args.device),
                                           self.players[playerid].G_orig.norm_adj.to(self.args.device))[self.players[
...
if __name__ == "__main__":
    # --dataset ogbn-arxiv --use_onehot --use_degree --use_entropy --ent 0.1 --policynet gat --maxbudget 5 --maxepisodes 1 --seed 102 --train_on_correct_only --k 3 --verbose --save_prefix inductive_non0_correct_only
    test_model = "GCN"
