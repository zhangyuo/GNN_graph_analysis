...
from source.src.GraphTransformer import load_GraphTransforer_model
# import wandb
...
checkpoint_res_file = open(
    '../results/{}/{}_train_checkpoint_{}.pkl'.format(args.dataset, args.dataset, args.save_prefix), 'wb')
...
def transfer_weights(dr_model, pyg_model, gcn_layer):
    print("Verification of the PyG model structure:")
    print(f"conv1.lin exists: {hasattr(pyg_model.conv1, 'lin')}")  # Should be True
    print(f"conv1.lin.weight shapes: {pyg_model.conv1.lin.weight.shape}")
    print(f"DeepRobust gc1.weight shapes: {dr_model.gc1.weight.shape}")
...
class SingleTrain(object):
    def __init__(self, args):
        ...
        if (self.dataset not in ['syn1', 'syn4', 'syn5', 'cora', 'chameleon', 'BA-SHAPES', 'TREE-CYCLES',
                                 'Loan-Decision', 'ogbn-arxiv']):
        ...
        if self.dataset == "cora":
            ...
            _data, adj, features, labels, pyg_data = data_obj.loadLoanDecision()
        ...
        file_path = f'../models/{test_model}/{dataset_name}/{gcn_layer}-layer/graphTransformer_model.pth'
        self.model = load_GraphTransforer_model(file_path, _data["data"], nhid, dropout, device, lr, weight_decay,
                                                gcn_layer,
                                                heads_num)
        ...
        if (self.args.train_on_non_zero):
            ...
            targets = np.random.choice(self.graph.idx_train, 200, replace=False).tolist()
            self.targets = torch.tensor(targets).unsqueeze(1)
        self.players = []
        self.player_idx = []
        i = 0
        for t in tqdm(self.targets, desc="Player Generate for target node"):
    ...
    def train(self):
        ...
        for episode in tqdm(range(0, self.args.maxepisodes), desc="Train process"):  # change made
    ...
    def playOneEpisode(self, playerid):
        ...
        try:
            orig_out = torch.argmax(self.model(self.players[playerid].G_orig.feats.to(self.args.device),
                                               self.players[playerid].G_orig.norm_adj.to(self.args.device))[
                                        self.players[
                                            playerid].G_orig.target_idx]).item()  # self.graph.labels[self.players[playerid].target]#
            curr_out = torch.argmax(self.model(self.players[playerid].G_curr.feats.to(self.args.device),
                                               self.players[playerid].G_curr.norm_adj.to(self.args.device))[
                                        self.players[playerid].G_curr.target_idx]).item()
        except:
            norm_adj = self.players[playerid].G_orig.norm_adj.to(self.args.device)
            edge_index_new, edge_weight_new = dense_to_sparse(norm_adj)
            orig_out = torch.argmax(self.model(self.players[playerid].G_orig.feats.to(self.args.device),
                                               edge_index=edge_index_new, edge_weight=edge_weight_new)[self.players[
                playerid].G_orig.target_idx]).item()  # self.graph.labels[self.players[playerid].target]#
            norm_adj = self.players[playerid].G_curr.norm_adj.to(self.args.device)
            edge_index_new, edge_weight_new = dense_to_sparse(norm_adj)
            curr_out = torch.argmax(self.model(self.players[playerid].G_curr.feats.to(self.args.device),
                                               edge_index=edge_index_new, edge_weight=edge_weight_new)[
                                        self.players[playerid].G_curr.target_idx]).item()
        ...
        while (orig_out == curr_out and b < self.maxbudget and len(self.players[playerid].cand_dict) >= 1):
            ...
            _, logits = self.policy(state[0].to(self.args.device), state[1].to(self.args.device),
                                    self.players[playerid].cand_dict)
            ...
            reward, loss_pred, loss_graph_dist = self.env.step(action,
                                                               playerid)  # passing b as parameter to make beta as beta/sqrt(b) after b>=5
            if (self.args.verbose):
            ...
            try:
                curr_out = self.model(self.players[playerid].G_curr.feats.to(self.args.device),
                                      self.players[playerid].G_curr.norm_adj.to(self.args.device))[
                    self.players[playerid].G_curr.target_idx]
            except:
                norm_adj = self.players[playerid].G_curr.norm_adj.to(self.args.device)
                edge_index_new, edge_weight_new = dense_to_sparse(norm_adj)
                curr_out = self.model(self.players[playerid].G_curr.feats.to(self.args.device),
        ...
        if (orig_out != curr_out):
            ...
            for i in range(len(self.players[playerid].cf_cand)):  # made changes
                val = [self.players[playerid].G_curr.reverse_map[self.players[playerid].cf_cand[i][0][0]],
                       self.players[playerid].G_curr.reverse_map[self.players[playerid].cf_cand[i][0][1]],
                       self.players[playerid].cf_cand[i][1]]
            ...
            self.cf_dict[self.players[playerid].target.item()] = [self.players[playerid].G_curr.target_idx.item(),
                                                                  self.players[playerid].G_curr.adj,
                                                                  self.players[playerid].G_orig.adj, counterfactual,
                                                                  self.players[playerid].G_curr.adj.shape[0],
                                                                  self.players[playerid].G_curr.node_map, orig_out,
                                                                  curr_out, self.players[playerid].G.labels[
                                                                      self.players[playerid].target], reward, loss_pred,
                                                                  loss_graph_dist, 'found']  # self.players[playerid].cf
        else:
            counterfactual = []
            for i in range(len(self.players[playerid].cf_cand)):  # made changes
                val = [self.players[playerid].G_curr.reverse_map[self.players[playerid].cf_cand[i][0][0]],
            ...
            self.cf_dict[self.players[playerid].target.item()] = [self.players[playerid].G_curr.target_idx.item(),
                                                                  ...
                                                                  curr_out, self.players[playerid].G.labels[
                                                                      self.players[playerid].target], reward, loss_pred,
                                                                  loss_graph_dist,
                                                                  'not found']  # self.players[playerid].cf
            res_file.write('target: {}, cf: {}\n'.format(self.players[playerid].target.item(), []))
...
if __name__ == "__main__":
    # --dataset Loan-Decision --use_onehot --use_degree --use_entropy --ent 0.1 --policynet gat --maxbudget 5 --maxepisodes 5 --seed 102 --train_on_correct_only --k 3 --verbose --save_prefix inductive_non0_correct_only
    test_model = "GraphTransformer"
    dataset_name = "Loan-Decision"
    gcn_layer = 2
    nhid = 100
    dropout = 0
    device = 'cpu'
