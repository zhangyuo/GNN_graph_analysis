...
if __name__ == "__main__":
    ...
    cfexp_subgraph = subgraph_data['attacked_explanation_subgraph']

    # attack subgraph vs. attacked counterfactual explanation subgraph
    similarity_dict_4 = subgraph_quantify(attack_subgraph, cfexp_subgraph, pyg_data, pyg_gcn,
                                          graph_analysis_subgraph_path, target_node_list, target_node_list1,
                                          pic_name='at_vs_at-cf-ex', is_cf_explainer=True)

    # Save all results
    result4 = pd.DataFrame(similarity_dict_4.values(),
                           columns=['trained_gcn_test_state', 'attack_state', 'edge_attack_type', 'ged', 'mcs',
                                    'gev_at_ex',
                                    'gev_at_mcs', 'gev_ex_mcs'],
                           index=similarity_dict_4.keys())
    result4.to_csv(graph_analysis_subgraph_path + '/result_{}_{}_test.csv'.format('all', 'at_vs_at-cf-ex'))
