...
class GATNet(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels,
        ...
        # first floor
        self.layers.append(GATConv(in_channels, hidden_channels, heads=heads, dropout=dropout, edge_dim=None))

        # middle layer
        for _ in range(num_layers - 2):
            self.layers.append(
                GATConv(hidden_channels * heads, hidden_channels, heads=heads, dropout=dropout, edge_dim=None))

        # last layer
        self.layers.append(
            GATConv(hidden_channels * heads, out_channels, heads=1, concat=False, dropout=dropout, edge_dim=None))
