...
class Eval(object):
    def __init__(self, args):
        ...
        self.model = load_GraphTransforer_model(file_path, _data["data"], nhid, dropout, device, lr, weight_decay,gcn_layer, heads_num)
    ...
    def policyQuery(self, playerid=0):
        ...
        try:
            ...
            norm_adj = normalize_adj(self.players[playerid].G_orig.norm_adj.to(self.args.device))
            edge_index_new, edge_weight_new = dense_to_sparse(norm_adj)
            orig_out = torch.argmax(self.model(self.players[playerid].G_orig.feats.to(self.args.device), edge_index=edge_index_new, edge_weight=edge_weight_new)[self.players[playerid].G_orig.target_idx]).item()
            norm_adj = normalize_adj(self.players[playerid].G_curr.norm_adj.to(self.args.device))
            edge_index_new, edge_weight_new = dense_to_sparse(norm_adj)
            curr_out = torch.argmax(self.model(self.players[playerid].G_curr.feats.to(self.args.device), edge_index=edge_index_new, edge_weight=edge_weight_new)[self.players[playerid].G_curr.target_idx]).item()
        ...
        try:
            ...
            edge_index_new, edge_weight_new = dense_to_sparse(norm_adj)
            eval_data["prob_new_actual"] = self.model(self.players[playerid].G_curr.feats.to(self.args.device),
                                                      edge_index=edge_index_new, edge_weight=edge_weight_new)[
...
if __name__ == "__main__":
    # --dataset Loan-Decision --use_onehot --use_degree --use_entropy --ent 0.1 --policynet gat --maxbudget 5 --seed 102 --k 3  --verbose --save_prefix inductive_non0_correct_only
    ...
    weight_decay = 0.001
    heads_num = 2
    test_nodes = [0, 3, 6, 8, 15, 19, 21, 25, 28, 31, 42, 43, 44, 45, 46, 47, 48, 50, 54, 57, 63, 67, 69, 70, 72, 73, 76, 79, 80, 81, 84, 86, 91, 92, 93, 95, 96, 97, 98, 102, 108, 110, 112, 117, 119, 122, 125, 126, 129, 130, 131, 134, 139, 140, 142, 144, 145, 147, 151, 159, 160, 161, 162, 163, 165, 168, 173, 176, 180, 187, 193, 194, 195, 196, 197, 201, 202, 205, 207, 209, 210, 215, 216, 220, 221, 225, 228, 229, 230, 231, 238, 242, 243, 247, 249, 251, 252, 256, 257, 258, 259, 261, 262, 273, 274, 279, 280, 289, 291, 293, 294, 304, 308, 310, 312, 313, 320, 321, 322, 336, 339, 343, 344, 345, 346, 349, 350, 354, 355, 356, 358, 359, 363, 366, 372, 373, 375, 376, 378, 379, 380, 382, 388, 389, 390, 403, 404, 410, 414, 422, 423, 430, 435, 450, 458, 460, 468, 472, 478, 480, 484, 488, 489, 490, 493, 498, 509, 514, 516, 523, 524, 530, 533, 539, 554, 561, 567, 578, 590, 592, 593, 597, 601, 605, 610, 616, 619, 624, 627, 634, 638, 649, 650, 657, 665, 667, 671, 678, 687, 696, 700, 701, 705, 708, 709, 717, 731, 736, 741, 743, 745, 746, 753, 760, 768, 770, 774, 776, 781, 786, 789, 801, 804, 810, 815, 817, 831, 838, 842, 844, 847, 851, 853, 859, 864, 869, 878, 886, 893, 901, 905, 907, 918, 920, 924, 938, 941, 945, 954, 965, 967, 970, 971, 975, 977, 979, 981, 984, 992, 993, 996]
