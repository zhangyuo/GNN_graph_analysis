...
class Player(nn.Module):
    ...
    def setup(self):
        ...
        with torch.no_grad():
            ...
            try:
                ...
                norm_adj = normalize_adj(sub_adj).to(self.args.device)
                edge_index_new, edge_weight_new = dense_to_sparse(norm_adj)
                out_sub = self.net(sub_feat.to(self.args.device), edge_index=edge_index_new, edge_weight=edge_weight_new)
    ...
    def oneStepReward(self): #compute reward
        try:
            ...
            norm_adj = self.G_last.norm_adj.to(self.args.device)
            edge_index_new, edge_weight_new = dense_to_sparse(norm_adj)
            out_probs_last = self.net(self.G_last.feats.to(self.args.device), edge_index=edge_index_new, edge_weight=edge_weight_new)
            norm_adj = self.G_curr.norm_adj.to(self.args.device)
            edge_index_new, edge_weight_new = dense_to_sparse(norm_adj)
            out_probs_curr = self.net(self.G_curr.feats.to(self.args.device), edge_index=edge_index_new, edge_weight=edge_weight_new)
    ...
    def reset(self):
        ...
        try:
            ...
            norm_adj = self.G_orig.norm_adj
            edge_index_new, edge_weight_new = dense_to_sparse(norm_adj)
            self.allnodes_output = self.net(self.G_orig.feats, edge_index=edge_index_new, edge_weight=edge_weight_new).detach()
        self.G_last = Subgraph(self.G_orig.adj.detach().clone(), self.G_orig.feats.detach().clone(), self.G_orig.labels.detach().clone(), self.G_orig.node_map, self.G_orig.target_idx, self.G_orig.reverse_map)
        self.G_curr = Subgraph(self.G_orig.adj.detach().clone(), self.G_orig.feats.detach().clone(), self.G_orig.labels.detach().clone(), self.G_orig.node_map, self.G_orig.target_idx, self.G_orig.reverse_map)
        self.cand_dict = self.getCandidates() 
        self.cf_cand = [] 
